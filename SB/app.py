...
Feature History:
  1-10: Core tutoring (chat, diagnostics, lessons, practice, parent dashboard,
        multi-tenancy, customization, ad hoc diagnostics, feedback, session mgmt)
  11-12: Gamification (badges, XP & levels)
  13: Family invite links
  14-17: Adaptive learning, re-assessment, standards alignment, progress checkpoints
  18: Socratic mode
  19-20: Content safety & safety logging
  21: Study mode (flashcards & quiz)
  23: Product tour & interactive help
  24: Onboarding intake / self-service setup
  25-27: Admin dashboard, usage analytics, instance-scoped safety & conversation logs
  29D-G: Admin safety/moderation, invite mgmt, usage analytics, system health
  34: Book Mastery — chapter-by-chapter comprehension verification with bookshelf
  16: Schoolwork Upload — upload homework/tests/syllabi, AI analysis, practice generation, grade tracking
  22: Study Timer & Session Goals — Pomodoro-style timer with session targets, study streaks, XP rewards
"""
...
def log_api_call(input_tokens: int, output_tokens: int, model: str, duration_ms: float, error: str = None):
...
def call_claude(system: str, messages: list, max_tokens: int = 1024) -> str:
    """Safely call Claude API with error handling. Returns response text or raises HTTPException."""
    t0 = _time.time()
    try:
...
# ─── Feature 57: Guided Conversation Mode Prompts ──────────────
GUIDED_MODE_PROMPT = """

GUIDED CONVERSATION MODE — CONVERSATION SCAFFOLDING:
You are in Guided mode. The student may be unfamiliar with AI chat, so actively guide the conversation.

After EVERY response, you MUST include a JSON block at the very end with 2-3 suggested follow-up actions.
Format this EXACTLY as shown (on its own line, after your main response):

===FOLLOWUPS===
["suggested action 1", "suggested action 2", "suggested action 3"]
===END_FOLLOWUPS===

Follow-up suggestions should be:
- Short (3-8 words) and tappable as buttons
- Contextually relevant to what you just discussed
- Varied: mix "explain differently", "practice problem", "what's next?", "give an example"
- NEVER generic — always specific to the current topic

When the conversation starts, proactively suggest what to do rather than waiting for input.
When you detect the student understands a concept (correct answer, good explanation), suggest progression:
Include ===PROGRESSION_READY=== before your follow-ups if the student should move to the next concept.
"""

OPEN_MODE_PROMPT = """

OPEN CONVERSATION MODE:
The student is comfortable with AI. Respond naturally and let them lead.
After your response, include 2-3 subtle follow-up suggestions (same format as guided mode):

===FOLLOWUPS===
["suggested action 1", "suggested action 2"]
===END_FOLLOWUPS===

Keep suggestions less directive — frame them as options, not instructions.
"""

CONFUSION_PROMPT = """

CONFUSION HANDLER — The student pressed the "I'm confused" button.
Re-explain the last concept using a completely DIFFERENT approach:
- Try an analogy, real-world example, or visual description
- Use shorter, simpler sentences
- Break the concept into smaller pieces
- Ask a simple yes/no check-in question at the end
Do NOT repeat the same explanation with minor rewording.
"""

DEEP_CONFUSION_PROMPT = """

DEEP CONFUSION — The student has pressed "I'm confused" multiple times in a row.
The previous explanations aren't working. Switch to a guided practice approach:
- Give a very simple, concrete example first
- Walk through it step by step, pausing to ask the student simple questions
- Use the most basic vocabulary possible
- Keep each response very short (under 80 words)
- Suggest a practice problem they can try with your help
"""

SESSION_WARMUP_MSG = "Welcome! I'm Atlas, your learning guide. You can type anything you'd like to learn about, or tap one of the buttons below to get started. There's no wrong answer here — I'm here to help you explore!"


def get_conversation_mode(student_id: str, instance_id: str = None) -> str:
    """Feature 57: Get conversation mode for a student. Returns 'guided' or 'open'."""
    if not student_id:
        return "guided"
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return "guided"
    return student.get("conversation_mode", "guided")


def get_default_conversation_mode(instance_id: str) -> str:
    """Feature 57: Determine default conversation mode from instance AI proficiency."""
    config = load_instance_config(instance_id)
    proficiency = config.get("customization", {}).get("ai_proficiency", "medium")
    if proficiency == "high":
        return "open"
    return "guided"  # low and medium default to guided


def increment_session_count(student_id: str, instance_id: str = None):
    """Feature 57: Increment the student's session count for warm-up tracking."""
    student = load_student(student_id, instance_id=instance_id)
    if student:
        student["session_count"] = student.get("session_count", 0) + 1
        save_student(student_id, student, instance_id=instance_id)


# ─── Feature 58: Adaptive Response Complexity ──────────────────
COMPLEXITY_TIER_PROMPTS = {
    1: """
RESPONSE COMPLEXITY — TIER 1 (FOUNDATIONAL):
- Use SHORT sentences (8-12 words average). One idea per sentence.
- Use ONLY common, everyday vocabulary. If you must use a subject term, define it immediately in simple words.
- Cover ONE concept per response. Do not combine ideas.
- Use concrete examples, real-world analogies, and visual descriptions heavily.
- Show EVERY intermediate step in calculations or reasoning.
- Keep responses under ~100 words before asking "Does that make sense?" or a simple check-in question.
- Use bullet points, numbered steps, and clear visual breaks.
- Ask yes/no or multiple-choice questions more than open-ended ones.
""",
    2: """
RESPONSE COMPLEXITY — TIER 2 (DEVELOPING):
- Use moderate sentence length (12-18 words). Clear and direct.
- Introduce subject-specific vocabulary, but define new terms on first use in parentheses.
- Up to TWO related concepts per response.
- Mix concrete examples with brief explanations.
- Show key steps in reasoning, but skip obvious intermediate steps.
- Keep responses under ~150 words before a check-in.
- Use a mix of structured (lists, steps) and conversational formats.
""",
    3: """
RESPONSE COMPLEXITY — TIER 3 (PROFICIENT):
- Use natural sentence length. Subject vocabulary freely.
- Connect multiple concepts in a single response.
- Explanations can be more conceptual and less step-by-step.
- Use examples selectively — for new or difficult ideas, not every point.
- Responses can run ~200 words. Check-ins are conversational, not formulaic.
- Open-ended questions are fine. Trust the student to flag confusion.
""",
    4: """
RESPONSE COMPLEXITY — TIER 4 (ADVANCED):
- Full academic register. Abstract reasoning and synthesis across concepts.
- Socratic questioning and intellectual challenge are appropriate.
- Minimal scaffolding — expect the student to follow complex reasoning.
- Responses can be longer and more information-dense.
- Encourage the student to make connections, question assumptions, and think critically.
- Use academic vocabulary without definition unless truly obscure.
""",
}


def calculate_complexity_tier(student: dict, subject: str, instance_id: str = None) -> int:
    """Feature 58: Calculate complexity tier (1-4) for a student in a subject.
    Weighted: diagnostic 50%, grade 20%, goals 20%, proficiency 10%."""

    # Start with grade-based baseline (20% weight)
    ...
    # Diagnostic score (50% weight) — strongest signal
    diag_score = 2.5  # default if no diagnostic
    # Check for diagnostic profile
    profiles_dir = None
    if instance_id:
        profiles_dir = get_instance_students_dir(instance_id) / student["student_id"] / "diagnostics"
    if profiles_dir and profiles_dir.exists():
        profile_path = profiles_dir / f"{subject}_profile.json"
        if profile_path.exists():
            profile = safe_json_load(profile_path, default={})
            overall = profile.get("overall_score", None)
    ...
    # Learning goals (20% weight)
    goal_score = 2.5  # neutral default
    if instance_id:
        config = load_instance_config(instance_id)
        goals = config.get("customization", {}).get("learning_goals", {}).get("categories", [])
        if "struggling" in goals:
            goal_score = 1.5
        elif "enrichment" in goals:
            goal_score = 3.5
        elif "test_prep" in goals:
            goal_score = 3.0
        elif "reinforcing" in goals:
            goal_score = 2.0

    # AI proficiency (10% weight)
    prof_score = 2.5
    if instance_id:
        config = load_instance_config(instance_id)
        proficiency = config.get("customization", {}).get("ai_proficiency", "medium")
        if proficiency == "low":
            prof_score = 1.5  # nudge down for response density
        elif proficiency == "high":
            prof_score = 3.0

    # Weighted combination
    weighted = (diag_score * 0.5) + (grade_score * 0.2) + (goal_score * 0.2) + (prof_score * 0.1)

    # Clamp to tier 1-4
    tier = max(1, min(4, round(weighted)))
    return tier


def get_student_complexity_tier(student: dict, subject: str, instance_id: str = None) -> int:
    """Feature 58: Get stored tier or calculate default."""
    tiers = student.get("complexity_tiers", {})
    if subject in tiers:
        return tiers[subject]
    # Calculate and store
    tier = calculate_complexity_tier(student, subject, instance_id)
    return tier


def save_complexity_tier(student_id: str, subject: str, tier: int, instance_id: str = None):
    """Feature 58: Store a complexity tier for a student/subject."""
    student = load_student(student_id, instance_id=instance_id)
    if student:
        if "complexity_tiers" not in student:
            student["complexity_tiers"] = {}
        student["complexity_tiers"][subject] = tier
        save_student(student_id, student, instance_id=instance_id)


def get_complexity_prompt(tier: int) -> str:
    """Feature 58: Return the complexity tier prompt for injection into system prompts."""
    return COMPLEXITY_TIER_PROMPTS.get(tier, COMPLEXITY_TIER_PROMPTS[2])
    ...
    if not config:
        config = {
            ...
            "customization": {
                ...
                "ai_proficiency": "medium",  # Feature 59: "low" | "medium" | "high"
                "feature_flags": {
...
BADGES = {
    ...
    "first_book_mastery": {"name": "Bookworm", "icon": "📖", "desc": "Completed your first Book Mastery"},
    "five_book_masteries": {"name": "Literary Scholar", "icon": "📚", "desc": "Mastered 5 books"},
}
...
XP_REWARDS = {
    ...
    "quiz_complete": 25,
    "quiz_perfect": 40,
    "book_mastery_chapter": 15,
    "book_mastery_complete": 50,
    "book_mastery_perfect": 75,
    "pomodoro_complete": 15,
    "pomodoro_goal_met": 25,
    "pomodoro_streak_bonus": 10,
}
...
class ChatRequest(BaseModel):
    ...
    confused: bool = False  # Feature 57: "I'm confused" button pressed
    confusion_count: int = 0  # Feature 57: consecutive confusion presses
...
def check_and_award_badges(student_id: str, instance_id: str = None) -> list:
    ...
    # Count completed book masteries
    book_mastery_count = 0
    bm_dir = student_data_dir(student_id, instance_id) / "book_mastery"
    if bm_dir.exists():
        for bf in bm_dir.glob("*.json"):
            bdata = safe_json_load(bf, default={})
            if bdata.get("status") == "complete":
                book_mastery_count += 1
    if book_mastery_count >= 1 and "first_book_mastery" not in earned:
        earned["first_book_mastery"] = {"earned_at": now}
        newly_earned.append("first_book_mastery")
    if book_mastery_count >= 5 and "five_book_masteries" not in earned:
        earned["five_book_masteries"] = {"earned_at": now}
        newly_earned.append("five_book_masteries")
...
# ── Feature 61: Topic Recommendation Engine thresholds ──
LEARN_THRESHOLD = 40          # Below this → Expedition (Learn)
MASTERY_THRESHOLD = 85        # Above this → Strong / Hard Practice
STUCK_PRACTICE_COUNT = 2      # Min practice attempts before stuck detection
STUCK_ACCURACY_THRESHOLD = 50 # Below this on repeated attempts → tutor
...
Begin now with Step 1: Hook. Make it engaging!"""
    ...
    # Feature 57: Add follow-up chip generation to lessons
    prompt += """

After EVERY response, include follow-up suggestions for the student:
===FOLLOWUPS===
["suggestion 1", "suggestion 2", "suggestion 3"]
===END_FOLLOWUPS===
Suggestions should guide the student through the lesson (e.g., "I understand, next step", "Can you explain that differently?", "Give me a practice problem")."""
    # Feature 58: Apply complexity tier to lesson prompts
    if student_id:
        student = load_student(student_id, instance_id=instance_id)
        if student:
            tier = get_student_complexity_tier(student, subject, instance_id)
            prompt += get_complexity_prompt(tier)
...
def build_practice_system_prompt(subject: str, topic: str, difficulty: str, profile: dict | None, student_name: str = None, instance_id: str = None, grade: int = 8, student_id: str = None) -> str:
    config = resolve_subject(subject, instance_id) or SUBJECTS.get(subject, {})
    name_line = ""
    if student_name:
...
After EVERY response, include follow-up suggestions:
===FOLLOWUPS===
["suggestion 1", "suggestion 2"]
===END_FOLLOWUPS===
Suggestions should help the student (e.g., "Give me a hint", "Next problem", "Explain that answer")."""
    # Feature 58: Apply complexity tier to practice prompts
    if student_id:
        student = load_student(student_id, instance_id=instance_id)
        if student:
            tier = get_student_complexity_tier(student, subject, instance_id)
...
async def api_create_student(request: StudentCreateRequest, instance_id: str = None):
    ...
    student_data = {
        ...
        "conversation_mode": get_default_conversation_mode(instance_id),  # Feature 57
        "session_count": 0,  # Feature 57: tracks sessions for warm-up
        "complexity_tiers": {},  # Feature 58: per-subject complexity tiers
    }
...
async def chat(request: ChatRequest):
    ...
    if sid:
        ...
        if student:
            ...
            conv_mode = student.get("conversation_mode", "guided")
        ...
        # Feature 57: Apply conversation mode prompt
        if conv_mode == "guided":
            system += GUIDED_MODE_PROMPT
        else:
            system += OPEN_MODE_PROMPT
        # Feature 57: Handle confusion signals
        if request.confused:
            if request.confusion_count >= 2:
                system += DEEP_CONFUSION_PROMPT
            else:
                system += CONFUSION_PROMPT
        # Feature 58: Apply complexity tier
        if student:
            tier = get_student_complexity_tier(student, request.subject, instance_id)
            # In-session adjustment: confusion drops tier temporarily
            if request.confused and tier > 1:
                tier = max(1, tier - 1)
            system += get_complexity_prompt(tier)
    else:
        system = wrap_atlas_voice(system, grade=grade)
        system += GUIDED_MODE_PROMPT  # Default to guided for guests
        system += get_complexity_prompt(2)  # Default tier 2 for guests

    # Apply content safety rules to system prompt
    system = apply_safety_rules(system, request.subject, grade)

    # Feature 57: Session warm-up for new users (first 3 sessions)
    warmup_text = None
    if sid and session_count < 3 and len(messages) == 0:
        warmup_text = SESSION_WARMUP_MSG
        increment_session_count(sid, instance_id=instance_id)
    ...
    # Feature 57: Parse follow-up suggestions from response
    followups = []
    clean_text = assistant_text
    if "===FOLLOWUPS===" in assistant_text:
        parts = assistant_text.split("===FOLLOWUPS===")
        clean_text = parts[0].rstrip()
        if len(parts) > 1:
            followup_block = parts[1].split("===END_FOLLOWUPS===")[0].strip()
            try:
                followups = json.loads(followup_block)
            except (json.JSONDecodeError, ValueError):
                followups = []

    progression_ready = "===PROGRESSION_READY===" in clean_text
    clean_text = clean_text.replace("===PROGRESSION_READY===", "").strip()
...
async def practice_start(request: PracticeStartRequest):
    ...
    # Validate topic against grade-specific topics, config topics, AND any topic
    # the student already has diagnostic data for (handles grade changes gracefully)
    valid_topics = set(get_grade_topics(request.subject, grade) or config.get("topics", []))
    if profile and profile.get("topics"):
        valid_topics.update(profile["topics"].keys())
    ...
    system_prompt = build_practice_system_prompt(
        request.subject, request.topic, difficulty, profile, student_name, instance_id=instance_id, grade=grade, student_id=sid
    )
...
# ═══════════════════════════════════════════════════════════════
# Feature 34: Book Mastery — Chapter-by-Chapter Comprehension
# ═══════════════════════════════════════════════════════════════

class BookMasteryStartRequest(BaseModel):
    book_title: str
    author: str
    total_chapters: int
    student_id: str
...
class BookMasteryQuizRequest(BaseModel):
    session_id: str
    student_id: str
    instance_id: str = ""


class BookMasteryAnswerRequest(BaseModel):
    session_id: str
    student_id: str
    answers: list  # list of {"question_index": int, "answer": str}
    instance_id: str = ""


def _bm_dir(student_id: str, instance_id: str = None) -> Path:
    """Return the book_mastery directory for a student."""
    d = student_data_dir(student_id, instance_id) / "book_mastery"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _load_bm_session(session_id: str, student_id: str, instance_id: str = None) -> dict | None:
    path = _bm_dir(student_id, instance_id) / f"{session_id}.json"
    data = safe_json_load(path, default=None)
    return data if isinstance(data, dict) else None


def _save_bm_session(session_id: str, student_id: str, data: dict, instance_id: str = None):
    path = _bm_dir(student_id, instance_id) / f"{session_id}.json"
    tmp = path.with_suffix(".tmp")
    try:
        tmp.write_text(json.dumps(data, indent=2))
...
@app.post("/api/book-mastery/start")
async def book_mastery_start(request: BookMasteryStartRequest):
    """Start a new Book Mastery session for a book."""
    instance_id = request.instance_id or "default"
    sid = request.student_id
    student = load_student(sid, instance_id=instance_id)
    if not student:
    ...
    total_chapters = max(1, min(50, request.total_chapters))
    ...
    _save_bm_session(session_id, sid, session_data, instance_id)
    return {
        "session_id": session_id,
...
@app.post("/api/book-mastery/quiz")
async def book_mastery_quiz(request: BookMasteryQuizRequest):
    """Generate quiz questions for the current chapter of a Book Mastery session."""
    instance_id = request.instance_id or "default"
    sid = request.student_id
    session = _load_bm_session(request.session_id, sid, instance_id)
    if not session:
        return {"error": "Session not found"}
    if session["status"] == "complete":
        return {"error": "This book has already been mastered"}
    ...
    system = f"""You are a reading comprehension evaluator for a {grade}th grade student.

The student claims to have read "{book}" by {author}.
You are generating comprehension questions for CHAPTER {chapter} of {session['total_chapters']}.

RULES:
- Generate exactly 5 questions for this chapter
- Mix question types: 3 multiple-choice and 2 open-ended short-answer
- Questions MUST test actual reading comprehension — details only someone who read the chapter would know
- Include questions about: key events, character motivations, important details, cause-and-effect, and themes
- For multiple-choice: provide 4 choices (A-D), only one correct
- For open-ended: expect 1-3 sentence answers demonstrating understanding
- Do NOT include questions that could be answered from a summary alone — ask about specific details, dialogue, or scenes
- Keep language grade-appropriate
- If you are not familiar with the specific chapter breakdown of this book, create questions based on logical chapter divisions of the content

Respond with ONLY a JSON array:
[
  {{"type": "multiple_choice", "question": "...", "choices": ["A. ...", "B. ...", "C. ...", "D. ..."], "correct": 0, "explanation": "..."}},
  {{"type": "open_ended", "question": "...", "rubric": "A good answer should mention..."}},
  ...
]"""

    try:
        raw = call_claude(system, [{"role": "user", "content": f"Generate 5 comprehension questions for Chapter {chapter} of \"{book}\" by {author}."}], max_tokens=2500)
        json_match = re.search(r'\[.*\]', raw, re.DOTALL)
        if json_match:
            questions = json.loads(json_match.group())
        else:
...
@app.post("/api/book-mastery/answer")
async def book_mastery_answer(request: BookMasteryAnswerRequest):
    """Submit answers for the current chapter and get evaluation."""
    instance_id = request.instance_id or "default"
    sid = request.student_id
    session = _load_bm_session(request.session_id, sid, instance_id)
    if not session:
    ...
    # Build evaluation prompt with student answers
    answers_text = ""
    for i, ans in enumerate(request.answers):
        answers_text += f"\nQuestion {i+1} — Student's answer: {ans.get('answer', '(no answer)')}"

    system = f"""You are evaluating a {grade}th grade student's reading comprehension answers for Chapter {chapter} of "{book}" by {author}.

EVALUATION RULES:
- For multiple-choice questions: mark correct or incorrect based on the correct answer
- For open-ended questions: evaluate whether the answer demonstrates genuine reading comprehension
- A student who actually read the chapter should be able to answer with specific details
- Vague or generic answers that could come from a summary should be marked as insufficient
- Be encouraging but honest — the goal is to verify the book was read
- A score of 70% or higher on a chapter means PASS
- Award partial credit for open-ended answers that show some understanding

Respond with ONLY a JSON object:
{{
  "chapter": {chapter},
  "results": [
    {{"question_index": 0, "correct": true, "feedback": "..."}},
    {{"question_index": 1, "correct": false, "partial_credit": 0.5, "feedback": "..."}},
    ...
  ],
  "score": 80,
  "passed": true,
  "chapter_feedback": "Overall feedback about their comprehension of this chapter..."
}}"""

    try:
        raw = call_claude(system, [{"role": "user", "content": f"Here are the student's answers for Chapter {chapter}:{answers_text}"}], max_tokens=2000)
        json_match = re.search(r'\{.*\}', raw, re.DOTALL)
        if json_match:
            evaluation = json.loads(json_match.group())
        else:
            return {"error": "Failed to evaluate answers. Please try again."}

        chapter_score = evaluation.get("score", 0)
        passed = evaluation.get("passed", chapter_score >= 70)

        # Update session
        chapter_key = f"chapter_{chapter}"
        session["chapters"][chapter_key] = {
            "score": chapter_score,
            "passed": passed,
            "attempts": session["chapters"].get(chapter_key, {}).get("attempts", 0) + 1,
            "completed_at": datetime.now().isoformat(),
        }

        # Track totals
        q_count = len(request.answers)
        correct_count = sum(1 for r in evaluation.get("results", []) if r.get("correct"))
        session["total_questions"] += q_count
        session["total_correct"] += correct_count

        # Award chapter XP if passed
        xp_result = {}
        if passed:
            xp_result = award_xp(sid, XP_REWARDS["book_mastery_chapter"], "book_mastery_chapter", instance_id)

            # Move to next chapter or complete
            if chapter >= session["total_chapters"]:
                # All chapters done — compute overall score
                chapter_scores = [ch.get("score", 0) for ch in session["chapters"].values()]
                overall = round(sum(chapter_scores) / len(chapter_scores)) if chapter_scores else 0
                session["overall_score"] = overall
                session["status"] = "complete"
                session["completed_at"] = datetime.now().isoformat()

                # Award completion XP
                xp_result = award_xp(sid, XP_REWARDS["book_mastery_complete"], "book_mastery_complete", instance_id)

                # Perfect bonus (all chapters 90%+)
                if all(s >= 90 for s in chapter_scores):
                    xp_result = award_xp(sid, XP_REWARDS["book_mastery_perfect"], "book_mastery_perfect", instance_id)

                # Save session BEFORE badge check so disk state is current
                _save_bm_session(request.session_id, sid, session, instance_id)
        ...
        # Log conversation
        log_conversation_turn(sid, "book_mastery", "book_mastery",
                              f"Chapter {chapter} answers for '{book}'",
                              f"Score: {chapter_score}%, Passed: {passed}")
        ...
        if session["status"] == "complete":
            ...
            response["book_title"] = session["book_title"]
            response["completed"] = True

        if passed and chapter < session["total_chapters"]:
            response["next_chapter"] = chapter + 1
...
@app.post("/api/book-mastery/retry")
async def book_mastery_retry(request: BookMasteryQuizRequest):
    """Retry the current chapter if the student didn't pass."""
    instance_id = request.instance_id or "default"
    session = _load_bm_session(request.session_id, request.student_id, instance_id)
    if not session:
        return {"error": "Session not found"}
    if session["status"] == "complete":
        return {"error": "This book has already been mastered"}

    # Just generate new questions for the same chapter
    return await book_mastery_quiz(request)


@app.post("/api/book-mastery/summary")
async def book_mastery_summary(request: BookMasteryQuizRequest):
    """Generate a book summary after the student has demonstrated mastery."""
    instance_id = request.instance_id or "default"
    sid = request.student_id
    session = _load_bm_session(request.session_id, sid, instance_id)
    if not session:
        return {"error": "Session not found"}
    if session["status"] != "complete":
        return {"error": "You must complete all chapter quizzes before receiving a summary"}
    ...
    system = f"""You are an encouraging reading tutor for a {grade}th grade student who has just demonstrated strong comprehension of "{book}" by {author}.

Write a thoughtful book summary and congratulatory message:
1. Start with congratulations — acknowledge their achievement
2. Provide a well-written summary of the book (3-4 paragraphs) covering plot, themes, and key takeaways
3. End with a reflection prompt — ask a thought-provoking question about the book's themes or relevance

Keep language grade-appropriate and encouraging. This is a reward for their hard work reading the book."""

    try:
        summary = call_claude(system, [{"role": "user", "content": f"Generate the summary and congratulations for completing \"{book}\" by {author}. Overall comprehension score: {session['overall_score']}%."}], max_tokens=1500)
        return {
            "session_id": request.session_id,
            "book_title": book,
            "author": author,
...
@app.get("/api/book-mastery/bookshelf/{student_id}")
async def book_mastery_bookshelf(student_id: str, instance_id: str = "default"):
    """Get the student's bookshelf — all books with mastery status."""
    bm_dir = _bm_dir(student_id, instance_id)
    books = []
    for f in sorted(bm_dir.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True):
        data = safe_json_load(f, default=None)
        if isinstance(data, dict):
            books.append({
    ...
    return {"bookshelf": books, "total_books": len(books), "completed": sum(1 for b in books if b["status"] == "complete")}


@app.get("/api/book-mastery/session/{session_id}")
async def book_mastery_session_detail(session_id: str, student_id: str = "", instance_id: str = "default"):
    """Get details of a specific Book Mastery session."""
    if not student_id:
        return {"error": "student_id required"}
    session = _load_bm_session(session_id, student_id, instance_id)
    if not session:
        return {"error": "Session not found"}
    return session
...
class SetupRequest(BaseModel):
    ...
    invite_code: str = ""
    state: str = ""
    ai_proficiency: str = "medium"  # "low" | "medium" | "high" — Feature 59
...
async def self_service_setup(request: SetupRequest):
    ...
    # 4b. Store AI proficiency level (Feature 59)
    if request.ai_proficiency in ("low", "medium", "high"):
        config["customization"]["ai_proficiency"] = request.ai_proficiency
    save_instance_config(instance_id, config)

    # 5. Create first student
    student_id = uuid.uuid4().hex[:8]
    now = datetime.now().isoformat()
    # Determine default conversation mode from AI proficiency (Feature 57)
    default_mode = "guided" if request.ai_proficiency != "high" else "open"
    student_data = {
        "student_id": student_id,
        ...
        "preferences": {},
        "activity_dates": [datetime.now().strftime("%Y-%m-%d")],
        "conversation_mode": default_mode,  # Feature 57
        "session_count": 0,  # Feature 57
        "complexity_tiers": {},  # Feature 58
    }
...
@app.get("/api/instance/{instance_id}/ux-settings")
async def get_instance_ux_settings(instance_id: str):
    """Feature 59: Public endpoint returning UX-relevant instance settings (no PIN required).
    Used by student UI to adapt experience based on AI proficiency level."""
    config = load_instance_config(instance_id)
    customization = config.get("customization", {})
    return {
        "ai_proficiency": customization.get("ai_proficiency", "medium"),
        "display_name": config.get("display_name", "Atlas"),
    ...
    for f in students_dir.glob("*.json"):
        ...
        if isinstance(data, dict):
            students.append({
                ...
                "pin": data.get("pin", ""),
                "created_at": data.get("created_at"),
                "conversation_mode": data.get("conversation_mode", "guided"),
            })
...
@app.put("/api/instance/{instance_id}/student/{student_id}/conversation-mode")
async def set_conversation_mode(instance_id: str, student_id: str, pin: str, mode: str = "guided"):
    """Feature 57: Parent toggles conversation mode (guided/open) per student."""
    if not validate_instance_parent_pin(instance_id, pin):
        return JSONResponse(status_code=401, content={"error": "Unauthorized"})
    if mode not in ("guided", "open"):
        return JSONResponse(status_code=400, content={"error": "Mode must be 'guided' or 'open'"})
    student = load_instance_student(instance_id, student_id)
    if not student:
        return {"error": "Student not found"}
    student["conversation_mode"] = mode
    save_instance_student(instance_id, student_id, student)
    return {"status": "ok", "conversation_mode": mode}


@app.get("/api/instance/{instance_id}/student/{student_id}/complexity-tiers")
async def get_complexity_tiers(instance_id: str, student_id: str, pin: str):
    """Feature 58: Get complexity tiers for all subjects for a student."""
    if not validate_instance_parent_pin(instance_id, pin):
        return JSONResponse(status_code=401, content={"error": "Unauthorized"})
    student = load_instance_student(instance_id, student_id)
    if not student:
    ...
    subjects = get_enabled_subjects(instance_id)
    tiers = {}
    tier_labels = {1: "Foundational", 2: "Developing", 3: "Proficient", 4: "Advanced"}
    for subj_key in subjects:
        tier = get_student_complexity_tier(student, subj_key, instance_id)
        tiers[subj_key] = {"tier": tier, "label": tier_labels.get(tier, "Unknown")}
    return {"student_id": student_id, "tiers": tiers}
...
def _schoolwork_dir(student_id: str, instance_id: str = None) -> Path:
    """Return the schoolwork directory for a student."""
    d = student_data_dir(student_id, instance_id) / "schoolwork"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _uploads_dir(student_id: str, instance_id: str = None) -> Path:
    """Return the uploads subdirectory for raw files."""
    d = _schoolwork_dir(student_id, instance_id) / "uploads"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _load_schoolwork_index(student_id: str, instance_id: str = None) -> list:
    """Load the upload index (list of all uploads) for a student."""
    path = _schoolwork_dir(student_id, instance_id) / "index.json"
    data = safe_json_load(path, default=[])
    return data if isinstance(data, list) else []


def _save_schoolwork_index(student_id: str, uploads: list, instance_id: str = None):
    path = _schoolwork_dir(student_id, instance_id) / "index.json"
    tmp = path.with_suffix(".tmp")
    try:
...
def _load_upload_detail(upload_id: str, student_id: str, instance_id: str = None) -> dict | None:
    path = _schoolwork_dir(student_id, instance_id) / f"{upload_id}.json"
    data = safe_json_load(path, default=None)
    return data if isinstance(data, dict) else None


def _save_upload_detail(upload_id: str, student_id: str, data: dict, instance_id: str = None):
    path = _schoolwork_dir(student_id, instance_id) / f"{upload_id}.json"
    tmp = path.with_suffix(".tmp")
    try:
        tmp.write_text(json.dumps(data, indent=2))
...
def _load_curriculum(student_id: str, instance_id: str = None) -> dict:
    """Load curriculum sync data (syllabus topics, upcoming schedule)."""
    path = _schoolwork_dir(student_id, instance_id) / "curriculum.json"
    data = safe_json_load(path, default={})
    if not isinstance(data, dict):
        data = {}
    if "topics" not in data:
        data["topics"] = []
    if "syllabi" not in data:
        data["syllabi"] = []
    return data


def _save_curriculum(student_id: str, data: dict, instance_id: str = None):
    path = _schoolwork_dir(student_id, instance_id) / "curriculum.json"
    tmp = path.with_suffix(".tmp")
    try:
        tmp.write_text(json.dumps(data, indent=2))
...
def _load_grade_book(student_id: str, instance_id: str = None) -> list:
    """Load the grade tracking records."""
    path = _schoolwork_dir(student_id, instance_id) / "grades.json"
    data = safe_json_load(path, default=[])
    return data if isinstance(data, list) else []


def _save_grade_book(student_id: str, grades: list, instance_id: str = None):
    path = _schoolwork_dir(student_id, instance_id) / "grades.json"
    tmp = path.with_suffix(".tmp")
    try:
        tmp.write_text(json.dumps(grades, indent=2))
        tmp.rename(path)
    except IOError:
        if tmp.exists():
            tmp.unlink()
...
def _extract_docx_text(file_bytes: bytes) -> str:
    """Extract text from a DOCX file using zipfile (no external deps)."""
    import io
    import zipfile
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(file_bytes)) as zf:
            ...
            # Extract all text elements from Word XML
...
async def _analyze_upload_with_ai(file_bytes: bytes, file_type: str, filename: str,
                                   upload_type: str, grade: int, subject_hint: str = "") -> dict:
    """Use Claude to analyze uploaded schoolwork content."""
    ...
    if file_type == "image":
        ...
        # For PDFs, use Claude's vision on the PDF bytes
        b64 = base64.b64encode(file_bytes).decode("utf-8")
        content_parts.append({
            "type": "document",
        ...
        content_parts.append({"type": "text", "text": f"Document content:\n\n{text[:8000]}"})

    type_instructions = {
        "homework": "This is a homework assignment. Extract the specific questions/problems, identify the subject and topics, estimate difficulty level.",
        "test": "This is a test or quiz. Identify the tested skills and topics, question types (multiple choice, short answer, etc.), and difficulty level.",
        "graded": "This is graded work. Find the score/grade if visible, identify which questions were missed, and note the topics of missed questions.",
        "syllabus": "This is a course syllabus. Extract the course subject, list of topics in order, any due dates or schedule, and the overall curriculum structure.",
        "textbook": "This is a textbook page. Identify the subject, the specific topic being taught, key concepts, and vocabulary terms.",
    }

    instruction = type_instructions.get(upload_type, type_instructions["homework"])
    subject_clause = f" The student believes this is for: {subject_hint}." if subject_hint else ""

    content_parts.append({
        "type": "text",
        "text": f"""Analyze this uploaded schoolwork for a {grade}th grade student.{subject_clause}

{instruction}

Respond in STRICT JSON format with these fields:
{{
  "subject": "Math|Science|English|Social Studies|Other",
  ...
  "score": <number 0-100 or null if not graded work>,
  "missed_topics": ["topic1", ...] (only for graded work, topics the student got wrong),
  "curriculum_topics": [  (only for syllabus)
    {{"topic": "name", "order": 1, "estimated_week": "Week 1-2 or null"}}
  ]
}}
ONLY output valid JSON, nothing else."""
    })

    system = f"You are a schoolwork analysis assistant for Atlas, an AI tutoring platform. You analyze uploaded school documents to help personalize a {grade}th grade student's learning. Always respond in valid JSON."
    ...
    try:
        ...
        return {"error": "AI analysis returned invalid format", "raw": result_text[:500]}
    except Exception as e:
        duration_ms = (_time.time() - t0) * 1000
        log_api_call(0, 0, "claude-sonnet-4-20250514", duration_ms, error=type(e).__name__)
        log_error(type(e).__name__, str(e)[:300], context="schoolwork_analyze")
        return {"error": f"Analysis failed: {type(e).__name__}"}


@app.post("/api/schoolwork/upload")
async def schoolwork_upload(
    file: UploadFile = File(...),
    student_id: str = Form(...),
    instance_id: str = Form("default"),
    upload_type: str = Form("homework"),
    subject_hint: str = Form(""),
):
    """Upload a schoolwork file for AI analysis.
    upload_type: homework | test | graded | syllabus | textbook
    """
    # Validate student
    student = load_student(student_id, instance_id=instance_id)
    if not student:
    ...
    if content_type not in ALLOWED_UPLOAD_TYPES:
        return JSONResponse(status_code=400, content={
            "error": f"Unsupported file type: {content_type}. Supported: PDF, PNG, JPG, DOCX"
        })

    # Validate upload type
    valid_types = ("homework", "test", "graded", "syllabus", "textbook")
    ...
    if len(file_bytes) > MAX_UPLOAD_SIZE:
        return JSONResponse(status_code=400, content={
            "error": f"File too large ({len(file_bytes) // (1024*1024)}MB). Maximum: 10MB."
        })
    if len(file_bytes) == 0:
        return JSONResponse(status_code=400, content={"error": "Empty file"})

    file_type = ALLOWED_UPLOAD_TYPES[content_type]
    grade = student.get("grade", 8)

    # Save the raw file
    upload_id = str(uuid.uuid4())[:12]
    ext = Path(file.filename or "upload").suffix or f".{file_type}"
    saved_filename = f"{upload_id}{ext}"
    upload_path = _uploads_dir(student_id, instance_id) / saved_filename
    upload_path.write_bytes(file_bytes)

    # Run AI analysis
    analysis = await _analyze_upload_with_ai(
        file_bytes, file_type, file.filename or "upload",
        upload_type, grade, subject_hint
    )
    ...
    # Save detail file
    _save_upload_detail(upload_id, student_id, upload_record, instance_id)

    # Add to index
    index = _load_schoolwork_index(student_id, instance_id)
    index_entry = {
        "upload_id": upload_id,
        "filename": file.filename or "upload",
        "upload_type": upload_type,
    ...
    index.insert(0, index_entry)  # newest first
    _save_schoolwork_index(student_id, index, instance_id)

    # Handle graded work — auto-record grade
    if upload_type == "graded" and analysis.get("score") is not None:
        grade_record = {
            "upload_id": upload_id,
            "subject": analysis.get("subject", "Unknown"),
            "score": analysis["score"],
            "topics": analysis.get("topics", []),
            "missed_topics": analysis.get("missed_topics", []),
            "recorded_at": datetime.now().isoformat(),
            "filename": file.filename or "upload",
        }
        grades = _load_grade_book(student_id, instance_id)
        grades.insert(0, grade_record)
        _save_grade_book(student_id, grades, instance_id)

    # Handle syllabus — update curriculum
    if upload_type == "syllabus" and analysis.get("curriculum_topics"):
        curriculum = _load_curriculum(student_id, instance_id)
        syllabus_entry = {
            "upload_id": upload_id,
            "subject": analysis.get("subject", "Unknown"),
            "topics": analysis["curriculum_topics"],
            "added_at": datetime.now().isoformat(),
        }
        curriculum["syllabi"].append(syllabus_entry)
        # Merge topics into the master topic list
        existing_names = {t["topic"] for t in curriculum["topics"]}
        for ct in analysis["curriculum_topics"]:
            if ct["topic"] not in existing_names:
                curriculum["topics"].append({
                    "topic": ct["topic"],
                    "subject": analysis.get("subject", "Unknown"),
                    "source": "syllabus",
                    "status": "upcoming",
                    "order": ct.get("order", 0),
                })
                existing_names.add(ct["topic"])
        _save_curriculum(student_id, curriculum, instance_id)

    return {
        "upload_id": upload_id,
        "analysis": analysis,
        "upload_type": upload_type,
        "message": "Upload analyzed successfully",
    }


@app.get("/api/schoolwork/uploads/{student_id}")
async def schoolwork_list(student_id: str, instance_id: str = "default"):
    """Get all uploads for a student."""
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return JSONResponse(status_code=404, content={"error": "Student not found"})
    index = _load_schoolwork_index(student_id, instance_id)
    return {"uploads": index, "total": len(index)}


@app.get("/api/schoolwork/upload/{upload_id}")
async def schoolwork_detail(upload_id: str, student_id: str = "", instance_id: str = "default"):
    """Get detailed analysis for a specific upload."""
    if not student_id:
        return JSONResponse(status_code=400, content={"error": "student_id required"})
    detail = _load_upload_detail(upload_id, student_id, instance_id)
    if not detail:
        return JSONResponse(status_code=404, content={"error": "Upload not found"})
    return {"upload": detail}


@app.delete("/api/schoolwork/upload/{upload_id}")
async def schoolwork_delete(upload_id: str, student_id: str = "", instance_id: str = "default"):
    ...
    saved_file = detail.get("saved_file", "")
    if saved_file:
        file_path = _uploads_dir(student_id, instance_id) / saved_file
        if file_path.exists():
            try:
                file_path.unlink()
            except OSError:
                pass  # File may be locked or permission denied; continue cleanup

    # Remove the detail file
    detail_path = _schoolwork_dir(student_id, instance_id) / f"{upload_id}.json"
    ...
    # Remove from index
    index = _load_schoolwork_index(student_id, instance_id)
    index = [u for u in index if u.get("upload_id") != upload_id]
    _save_schoolwork_index(student_id, index, instance_id)

    return {"deleted": upload_id}


@app.post("/api/schoolwork/practice")
async def schoolwork_practice(
    upload_id: str = Form(...),
    student_id: str = Form(...),
    instance_id: str = Form("default"),
    count: int = Form(5),
):
    """Generate practice problems based on an uploaded assignment."""
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return JSONResponse(status_code=404, content={"error": "Student not found"})
    ...
    question_types = analysis.get("question_types", ["problem_solving"])
    count = max(3, min(10, count))

    system = f"""You are Atlas, an AI tutor generating practice problems for a {grade}th grade student.
Generate practice problems that are SIMILAR to (but not identical to) the student's uploaded {detail.get('upload_type', 'homework')}.

Subject: {subject}
Topics: {', '.join(topics) if topics else 'General'}
Difficulty: {difficulty}
Original question types: {', '.join(question_types)}

Generate exactly {count} practice problems. Each problem should test the same skills but with different numbers, scenarios, or contexts.
...
ONLY output valid JSON."""
    ...
    try:
        result_text = call_claude(system, [{"role": "user", "content": f"Generate {count} practice problems similar to: {analysis.get('summary', 'uploaded assignment')}. Topics: {', '.join(topics)}"}], max_tokens=3000)
        ...
        # Mark practice as generated
        detail["practice_generated"] = True
        detail["practice_count"] = count
        detail["last_practice_at"] = datetime.now().isoformat()
        _save_upload_detail(upload_id, student_id, detail, instance_id)
        ...
        return JSONResponse(status_code=500, content={"error": "Failed to parse practice problems"})
    except HTTPException:
        raise
    except Exception as e:
        log_error(type(e).__name__, str(e)[:300], "schoolwork_practice")
        return JSONResponse(status_code=500, content={"error": f"Practice generation failed: {type(e).__name__}"})


@app.get("/api/schoolwork/curriculum/{student_id}")
async def schoolwork_curriculum(student_id: str, instance_id: str = "default"):
    """Get curriculum sync data for a student (from uploaded syllabi)."""
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return JSONResponse(status_code=404, content={"error": "Student not found"})
    curriculum = _load_curriculum(student_id, instance_id)
...
@app.get("/api/schoolwork/grades/{student_id}")
async def schoolwork_grades(student_id: str, instance_id: str = "default"):
    """Get grade tracking records for a student."""
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return JSONResponse(status_code=404, content={"error": "Student not found"})
    grades = _load_grade_book(student_id, instance_id)
    ...
    if grades:
        ...
        # Find most-missed topics
        missed = {}
        for g in grades:
            for t in g.get("missed_topics", []):
    ...
    return {
        "grades": grades,
        "summary": {
            "total_recorded": len(grades),
            "average_score": avg,
            "highest": max(scores) if scores else None,
            "lowest": min(scores) if scores else None,
            "most_missed_topics": [{"topic": t, "count": c} for t, c in top_missed],
        }
    }


@app.post("/api/schoolwork/grade/record")
async def schoolwork_record_grade(
    student_id: str = Form(...),
    instance_id: str = Form("default"),
    subject: str = Form(...),
    score: float = Form(...),
    ...
    topics: str = Form(""),
    missed_topics: str = Form(""),
):
    """Manually record a grade (without file upload)."""
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return JSONResponse(status_code=404, content={"error": "Student not found"})
    ...
    grade_record = {
        ...
        "missed_topics": [t.strip() for t in missed_topics.split(",") if t.strip()] if missed_topics else [],
        "recorded_at": datetime.now().isoformat(),
        "manual": True,
    }
    grades = _load_grade_book(student_id, instance_id)
    ...
    return {"recorded": True, "grade": grade_record}


# ═══════════════════════════════════════════════════════════════
# Feature 22: Study Timer & Session Goals (Pomodoro Timer)
# ═══════════════════════════════════════════════════════════════

def _study_sessions_path(student_id: str, instance_id: str = None) -> Path:
    """Return the path to the student's study_sessions.json file."""
    base = student_data_dir(student_id, instance_id)
    return base / "study_sessions.json"


def _load_study_sessions(student_id: str, instance_id: str = None) -> list:
    """Load study sessions for a student."""
    path = _study_sessions_path(student_id, instance_id)
    data = safe_json_load(path, default=[])
    return data if isinstance(data, list) else []


def _save_study_sessions(student_id: str, sessions: list, instance_id: str = None):
    """Save study sessions for a student."""
    path = _study_sessions_path(student_id, instance_id)
    tmp = path.with_suffix(".tmp")
    try:
...
class TimerStartRequest(BaseModel):
    student_id: str
    subject: str = ""
    goal_type: str = ""           # "problems", "questions", "pages", "custom"
    goal_target: int = 0          # number to complete
    topic_focus: str = ""         # optional specific topic
    study_minutes: int = 25       # planned study duration
    instance_id: str = ""
...
class TimerCompleteRequest(BaseModel):
    student_id: str
    session_id: str
    duration_minutes: float       # actual duration studied
    goals_met: bool = False
    goals_achieved: int = 0       # how many of goal_target completed
    instance_id: str = ""


class TimerGoalRequest(BaseModel):
    student_id: str
    session_id: str
    goal_type: str = ""
    goal_target: int = 0
...
@app.post("/api/study/timer/start")
async def study_timer_start(request: TimerStartRequest):
    """Log a new study timer session start."""
    instance_id = request.instance_id or "default"
    sid = request.student_id
    if not sid:
        return {"error": "student_id required"}
    ...
    sessions = _load_study_sessions(sid, instance_id)
    sessions.append(record)
    _save_study_sessions(sid, sessions, instance_id)

    return {"status": "ok", "session_id": session_id, "record": record}


@app.post("/api/study/timer/complete")
async def study_timer_complete(request: TimerCompleteRequest):
    """Log study timer session completion and award XP."""
    instance_id = request.instance_id or "default"
    sid = request.student_id
    if not sid:
        return {"error": "student_id required"}
    ...
    record["duration_minutes"] = round(request.duration_minutes, 1)
    record["goals"]["achieved"] = request.goals_achieved

    # Calculate XP awards
    xp_events = []
    # Base: complete a Pomodoro session
    xp_events.append(("pomodoro_complete", XP_REWARDS["pomodoro_complete"]))

    # Bonus: met session goal
    goal_met = request.goals_met or (
        record["goals"]["target"] > 0 and request.goals_achieved >= record["goals"]["target"]
    )
    record["goals_met"] = goal_met
    if goal_met:
        xp_events.append(("pomodoro_goal_met", XP_REWARDS["pomodoro_goal_met"]))

    # Streak bonus: check if student has studied 3+ consecutive days
    today = datetime.now().strftime("%Y-%m-%d")
    study_dates = sorted(set(s.get("date", "") for s in sessions if s.get("completed")))
    ...
    for i in range(len(study_dates) - 1, 0, -1):
        d1 = datetime.strptime(study_dates[i], "%Y-%m-%d")
        d2 = datetime.strptime(study_dates[i - 1], "%Y-%m-%d")
        if (d1 - d2).days == 1:
            streak += 1
        else:
            break
    ...
    if streak >= 3:
        xp_events.append(("pomodoro_streak_bonus", XP_REWARDS["pomodoro_streak_bonus"]))
    ...
    return {
        ...
        "xp_result": xp_result,
        "xp_gained": total_xp,
        "xp_events": [{"reason": r, "amount": a} for r, a in xp_events],
        "streak": streak,
        "goal_met": goal_met,
        "new_badges": new_badges,
        "record": record,
...
@app.get("/api/study/timer/stats/{student_id}")
async def study_timer_stats(student_id: str, instance_id: str = "default"):
    """Get study timer statistics for a student."""
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return {"error": "Student not found"}

    sessions = _load_study_sessions(student_id, instance_id)
    completed = [s for s in sessions if s.get("completed")]

    today = datetime.now().strftime("%Y-%m-%d")
    today_sessions = [s for s in completed if s.get("date") == today]
    today_minutes = sum(s.get("duration_minutes", 0) for s in today_sessions)

    # This week (Mon-Sun)
    now = datetime.now()
    week_start = (now - timedelta(days=now.weekday())).strftime("%Y-%m-%d")
    week_sessions = [s for s in completed if s.get("date", "") >= week_start]
    week_minutes = sum(s.get("duration_minutes", 0) for s in week_sessions)

    # Study dates for streak calc
    study_dates = sorted(set(s.get("date", "") for s in completed))
    streak = 0
    if study_dates:
        # Check streak ending today or yesterday
        last = study_dates[-1]
    ...
    # Goals completed
    goals_set = len([s for s in completed if s.get("goals", {}).get("target", 0) > 0])
    goals_met = len([s for s in completed if s.get("goals_met") or s.get("goals", {}).get("achieved", 0) >= s.get("goals", {}).get("target", 1)])

    # Total XP from timer
    total_xp = sum(s.get("xp_earned", 0) for s in completed)

    # Subjects studied
    subjects_studied = list(set(s.get("subject", "") for s in completed if s.get("subject")))

    # Recent sessions (last 10)
    recent = sorted(completed, key=lambda s: s.get("started_at", ""), reverse=True)[:10]
...
@app.post("/api/study/timer/goal")
async def study_timer_goal(request: TimerGoalRequest):
    """Update session goal for an active timer session."""
    instance_id = request.instance_id or "default"
    sid = request.student_id
    if not sid:
        return {"error": "student_id required"}
    ...
    record["goals"]["type"] = request.goal_type or record["goals"]["type"]
    record["goals"]["target"] = request.goal_target or record["goals"]["target"]
    record["goals"]["topic_focus"] = request.topic_focus or record["goals"]["topic_focus"]

    _save_study_sessions(sid, sessions, instance_id)
    return {"status": "ok", "goals": record["goals"]}


# ── Feature 61: Topic Recommendation Engine ──────────────────────

def _topic_recommendation(subject: str, topic: str, mastery: dict, student_id: str, instance_id: str = None) -> dict:
    """Compute the recommended action for a single topic."""
    score = mastery["score"]
    tier = mastery["tier"]
    lesson_count = mastery.get("lesson_count", 0)
    practice_count = mastery.get("practice_count", 0)
    practice_accuracy = mastery.get("practice_accuracy")
    due = mastery.get("due_for_review", False)

    stuck = False
    stuck_detail = None
    tutor_context = None
    if practice_count >= STUCK_PRACTICE_COUNT and practice_accuracy is not None and practice_accuracy < STUCK_ACCURACY_THRESHOLD:
        stuck = True
        stuck_detail = f"Practiced {practice_count} times with {practice_accuracy:.0f}% accuracy — below {STUCK_ACCURACY_THRESHOLD}% threshold"
        tutor_context = (
            f"The student has practiced {topic} in {subject} {practice_count} times but accuracy "
            f"remains at {practice_accuracy:.0f}%. Focus the conversation on breaking down the "
            f"concepts they are struggling with. Use a different teaching approach than previous attempts."
        )

    if stuck:
        return {
            "recommended_mode": "tutor", "recommended_difficulty": None,
            "reasoning": f"You know this topic ({score:.0f}%) but keep getting tripped up. Let's talk through the approach together.",
            "alternative_mode": "practice", "alternative_label": "More practice",
            "badge": "tutor", "stuck": True, "stuck_detail": stuck_detail, "tutor_context": tutor_context,
        }

    if score < LEARN_THRESHOLD:
        return {
            "recommended_mode": "lesson", "recommended_difficulty": None,
            "reasoning": f"New territory ({score:.0f}%). Start with a guided expedition to learn the foundations.",
            "alternative_mode": "practice", "alternative_label": "Jump to practice",
            "badge": "learn", "stuck": False, "stuck_detail": None, "tutor_context": None,
        }

    if score < MASTERY_THRESHOLD:
        if ZERO_LESSON_OVERRIDE and lesson_count == 0:
            return {
                "recommended_mode": "lesson", "recommended_difficulty": None,
                "reasoning": f"You scored {score:.0f}% but haven't had structured teaching yet. Start with an expedition first.",
                "alternative_mode": "practice", "alternative_label": "Jump to practice",
                "badge": "learn", "stuck": False, "stuck_detail": None, "tutor_context": None,
            }
        difficulty = "hard" if score >= 70 else "medium"
        reason_suffix = " (due for review)" if due else ""
        return {
            "recommended_mode": "practice", "recommended_difficulty": difficulty,
            "reasoning": f"You've got the basics ({score:.0f}%). Drill with practice to build consistency.{reason_suffix}",
            "alternative_mode": "lesson", "alternative_label": "Review the concepts",
            "badge": "practice", "stuck": False, "stuck_detail": None, "tutor_context": None,
        }

    reason_suffix = " — due for a refresh" if due else ""
    return {
        "recommended_mode": "practice", "recommended_difficulty": "hard",
        "reasoning": f"Strong foundation ({score:.0f}%). Push toward mastery with harder problems.{reason_suffix}",
        "alternative_mode": "lesson", "alternative_label": "Explore advanced concepts",
        "badge": "strong", "stuck": False, "stuck_detail": None, "tutor_context": None,
    }


def _build_action_plan(student_id: str, instance_id: str = None, grade: int = 8, focus_subject: str = None) -> dict:
    """Build a prioritized action plan across all subjects for the My Map."""
    subjects = get_enabled_subjects(instance_id) if instance_id else SUBJECTS
    actions = []
    ...
    for subj in subjects:
        profile = load_profile(subj, student_id, instance_id=instance_id)
        config = resolve_subject(subj, instance_id) or SUBJECTS.get(subj, {})
        subj_icon = config.get("icon", "📚")
        subj_name = config.get("name", subj)

        if profile is None:
            diag_urgency = 6
            if focus_subject and subj == focus_subject:
                diag_urgency = max(1, diag_urgency - 3)
            actions.append({
                "subject": subj, "subject_icon": subj_icon, "subject_name": subj_name,
                "topic": None, "mode": "diagnostic",
                "reasoning": "Take a diagnostic to unlock personalized recommendations",
                "tutor_context": None, "urgency": diag_urgency, "badge": "assess", "score": None,
            })
            continue
        ...
        for topic, m in sm["topics"].items():
            rec = _topic_recommendation(subj, topic, m, student_id, instance_id)
            score = m["score"]
            due = m.get("due_for_review", False)

            if rec["stuck"]:
                urgency = 2
            elif m["tier"] in ("needs_work", "developing") and due:
                urgency = 3
            elif due:
                urgency = 4
            elif m["tier"] in ("needs_work", "developing"):
                urgency = 5
            elif m["tier"] == "advanced" and due:
                urgency = 7
            else:
                continue
            ...
            actions.append({
                "subject": subj, "subject_icon": subj_icon, "subject_name": subj_name,
                "topic": topic, "mode": rec["recommended_mode"],
                "reasoning": rec["reasoning"], "tutor_context": rec.get("tutor_context"),
                "urgency": urgency, "badge": rec["badge"], "score": score,
            })

    actions.sort(key=lambda a: (a["urgency"], a.get("score") or 0))
    return {"actions": actions[:10], "focus_subject": focus_subject}


@app.get("/api/subject/recommendations/{subject}")
async def subject_recommendations(subject: str, student_id: str, instance_id: str = "default"):
    """Get per-topic recommendations for a subject (Feature 61)."""
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return JSONResponse(status_code=404, content={"error": "Student not found"})
    ...
    profile = load_profile(subject, student_id, instance_id=instance_id)
    if profile is None:
        return {"subject": subject, "topics": {}, "priority_actions": [], "has_diagnostic": False,
                "message": "Take a diagnostic first to unlock recommendations."}
    ...
    for topic, m in sm["topics"].items():
        rec = _topic_recommendation(subject, topic, m, student_id, instance_id)
        topic_recs[topic] = {
            "score": m["score"], "tier": m["tier"],
            "recommended_mode": rec["recommended_mode"],
            "recommended_difficulty": rec.get("recommended_difficulty"),
            "reasoning": rec["reasoning"],
            "alternative_mode": rec["alternative_mode"],
            "alternative_label": rec["alternative_label"],
            "badge": rec["badge"],
            "lesson_count": m["lesson_count"], "practice_count": m["practice_count"],
            "practice_accuracy": m.get("practice_accuracy"),
            "due_for_review": m["due_for_review"],
            "stuck": rec["stuck"], "stuck_detail": rec.get("stuck_detail"),
            "tutor_context": rec.get("tutor_context"),
        }

        if rec["stuck"] or m["due_for_review"] or m["tier"] in ("needs_work", "developing"):
            urgency = 1 if rec["stuck"] else (2 if m["due_for_review"] and m["tier"] in ("needs_work", "developing") else (3 if m["due_for_review"] else 4))
            priority_actions.append({"topic": topic, "mode": rec["recommended_mode"], "reason": rec["reasoning"], "urgency": urgency})

    priority_actions.sort(key=lambda a: (a["urgency"],))

    return {
        "subject": subject, "topics": topic_recs, "priority_actions": priority_actions[:5],
        "has_diagnostic": True, "overall_score": sm["overall_score"], "overall_tier": sm["overall_tier"],
    }


@app.get("/api/recommendations/action-plan/{student_id}")
async def student_action_plan(student_id: str, instance_id: str = "default"):
    """Get top priority actions across all subjects for My Map (Feature 61)."""
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return JSONResponse(status_code=404, content={"error": "Student not found"})

    grade = student.get("grade", 8)
    focus_subject = student.get("focus_subject")
    plan = _build_action_plan(student_id, instance_id, grade, focus_subject)
    return plan
...
@app.post("/api/instance/{instance_id}/student/{student_id}/focus")
async def set_focus_subject(instance_id: str, student_id: str, req: FocusRequest, pin: str = None):
    """Set or clear the parent's Focus Subject for a student (Feature 64)."""
    inst_dir = Path("data/instances") / (instance_id or "default")
    parent_file = inst_dir / "parent_config.json"
    if parent_file.exists():
        parent_config = safe_json_load(parent_file, {})
        required_pin = parent_config.get("pin", "")
    ...
    enabled = get_enabled_subjects(instance_id)
    if req.focus_subject and req.focus_subject not in enabled:
        return JSONResponse(status_code=400, content={"error": f"Subject '{req.focus_subject}' is not enabled"})

    sf = inst_dir / "students" / f"{student_id}.json"
    if sf.exists():
        data = safe_json_load(sf, {})
        data["focus_subject"] = req.focus_subject
        with open(sf, "w") as f:
            json.dump(data, f, indent=2)

    return {
        "success": True, "student_id": student_id, "focus_subject": req.focus_subject,
        "message": f"Focus set to {req.focus_subject}" if req.focus_subject else "Focus cleared — Atlas will decide priorities"
    }


@app.get("/api/instance/{instance_id}/student/{student_id}/focus")
async def get_focus_subject(instance_id: str, student_id: str):
    """Get the current Focus Subject for a student (Feature 64)."""
    student = load_student(student_id, instance_id=instance_id)
    if not student:
        return JSONResponse(status_code=404, content={"error": "Student not found"})
    return {"student_id": student_id, "focus_subject": student.get("focus_subject")}
