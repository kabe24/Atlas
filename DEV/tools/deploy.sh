...
# ── Configuration ──────────────────────────────────────────
# ROOT = the platform parent directory that contains all environments
# (e.g., "AI Tutor Platform/" which contains PROD/, DEV/, SB/, DEPLOY/, etc.)
# From tools/deploy.sh: go up twice (tools → env_root → platform_root)
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ENV_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
ROOT="$(cd "$ENV_ROOT/.." && pwd)"

# Deployable code files (relative to environment root)
DEPLOYABLE_FILES=(
    "app.py"
    "atlas_voice.py"
    "atlas_ux_language.html"
    "requirements.txt"
    ".env.example"
    "VERSION"
    "static/index.html"
    ...
    "static/img/atlas-compass-rose.svg"
)
...
# Resolve paths — all environments are sibling directories under ROOT
resolve_path() {
    local env_name="$1"
    echo "$ROOT/$env_name"
}
...
if [ "$TARGET" = "PROD" ]; then
    ...
    BACKUP_DIR="$ROOT/PROD_BACKUP/archive/$TIMESTAMP"
    MIRROR_DIR="$ROOT/PROD_BACKUP"
    warn "Target is PROD. Backing up current PROD first..."
    if [ "$DRY_RUN" = true ]; then
        info "[DRY RUN] Would create timestamped backup at: $BACKUP_DIR"
        info "[DRY RUN] Would update PROD_BACKUP mirror (flat copy)"
    else
        mkdir -p "$BACKUP_DIR"

        # ── Helper: backup a single file to both locations ──
        backup_file() {
            local src="$1" dest_rel="$2"
            mkdir -p "$BACKUP_DIR/$(dirname "$dest_rel")"
            cp "$src" "$BACKUP_DIR/$dest_rel"
            mkdir -p "$MIRROR_DIR/$(dirname "$dest_rel")"
            cp "$src" "$MIRROR_DIR/$dest_rel"
        }

        # Backup deployable code files from current PROD
        for f in "${DEPLOYABLE_FILES[@]}"; do
            if [ -f "$TARGET_PATH/$f" ]; then
                backup_file "$TARGET_PATH/$f" "$f"
            fi
        done
        for d in "${DEPLOYABLE_DIRS[@]}"; do
            if [ -d "$TARGET_PATH/$d" ]; then
                mkdir -p "$BACKUP_DIR/$d" "$MIRROR_DIR/$d"
                cp -r "$TARGET_PATH/$d/." "$BACKUP_DIR/$d/"
                cp -r "$TARGET_PATH/$d/." "$MIRROR_DIR/$d/"
            fi
        done
        ...
        # Record what version was backed up
        echo "$TIMESTAMP — Pre-deploy backup of PROD $(cat "$TARGET_PATH/VERSION" 2>/dev/null || echo 'unknown')" > "$BACKUP_DIR/BACKUP_INFO.txt"
        echo "Last backup: $TIMESTAMP — PROD $(cat "$TARGET_PATH/VERSION" 2>/dev/null || echo 'unknown')" > "$MIRROR_DIR/LAST_BACKUP.txt"

        ok "Timestamped backup: $BACKUP_DIR"
        ok "PROD_BACKUP mirror updated (flat copy for quick rollback)"
    fi
    echo ""
fi
