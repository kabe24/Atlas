"""
Comprehensive API Test Suite for Atlas (Features 1-10, 16, 34, 57-59)
...
async def run_tests():
    ...
    print(f"\n{BOLD}ATLAS API TEST SUITE (Features 1-10, 16, 34, 57-59){RESET}")
    print(f"{BOLD}{'='*80}{RESET}\n")
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # ─────────────────────────────────────────────────────────────────
        print(f"\n{BOLD}FEATURE 16: SCHOOLWORK UPLOAD & CURRICULUM INTEGRATION{RESET}")
        print("─" * 80)

        # Test 40: GET /api/schoolwork/uploads/{student_id} - Empty list initially
        response = await client.get(
            f"/api/schoolwork/uploads/{runner.student_id}",
            params={"instance_id": runner.instance_id}
        )
        runner.add_result(test_num, f"GET /api/schoolwork/uploads/{runner.student_id} (empty)", response.status_code, 200, response.json())
        if response.status_code == 200:
            data = response.json()
            assert data.get("uploads") == [], f"Expected empty uploads list, got {data}"
        test_num += 1

        # Test 41: GET /api/schoolwork/grades/{student_id} - Empty grades initially
        response = await client.get(
            f"/api/schoolwork/grades/{runner.student_id}",
            params={"instance_id": runner.instance_id}
        )
        runner.add_result(test_num, f"GET /api/schoolwork/grades/{runner.student_id} (empty)", response.status_code, 200, response.json())
        test_num += 1

        # Test 42: GET /api/schoolwork/curriculum/{student_id} - Empty curriculum initially
        response = await client.get(
            f"/api/schoolwork/curriculum/{runner.student_id}",
            params={"instance_id": runner.instance_id}
        )
        runner.add_result(test_num, f"GET /api/schoolwork/curriculum/{runner.student_id} (empty)", response.status_code, 200, response.json())
        test_num += 1

        # Test 43: POST /api/schoolwork/upload - Upload a text file (as DOCX-type test)
        # We simulate a small text upload that will be processed
        # For testing without Claude API, we test the validation paths
        import io
        fake_file_content = b"This is a test homework file for algebra equations."
        response = await client.post(
            "/api/schoolwork/upload",
            data={
                "student_id": runner.student_id,
                "instance_id": runner.instance_id,
            ...
            files={"file": ("test_hw.png", io.BytesIO(fake_file_content), "image/png")},
        )
        upload_id = None
        if response.status_code == 200:
            upload_data = response.json()
            upload_id = upload_data.get("upload_id")
        runner.add_result(test_num, "POST /api/schoolwork/upload (image)", response.status_code, 200, response.json() if response.status_code == 200 else None)
        test_num += 1

        # Test 44: POST /api/schoolwork/upload - Reject unsupported file type
        response = await client.post(
            "/api/schoolwork/upload",
            data={
                "student_id": runner.student_id,
        ...
        runner.add_result(test_num, "POST /api/schoolwork/upload (reject .exe)", response.status_code, 400, response.json())
        test_num += 1

        # Test 45: GET /api/schoolwork/uploads/{student_id} - Should have 1 upload now
        if upload_id:
            response = await client.get(
                f"/api/schoolwork/uploads/{runner.student_id}",
                params={"instance_id": runner.instance_id}
            ...
            data = response.json()
            has_upload = len(data.get("uploads", [])) >= 1
            runner.add_result(test_num, f"GET /api/schoolwork/uploads/{runner.student_id} (has upload)", response.status_code, 200, data)
        test_num += 1

        # Test 46: GET /api/schoolwork/upload/{upload_id} - Get detail
        if upload_id:
            response = await client.get(
                f"/api/schoolwork/upload/{upload_id}",
                params={"student_id": runner.student_id, "instance_id": runner.instance_id}
            )
            runner.add_result(test_num, f"GET /api/schoolwork/upload/{upload_id} (detail)", response.status_code, 200, response.json())
        test_num += 1

        # Test 47: POST /api/schoolwork/grade/record - Manual grade entry
        response = await client.post(
            "/api/schoolwork/grade/record",
            data={
                "student_id": runner.student_id,
                "instance_id": runner.instance_id,
                "subject": "Math",
                ...
                "topics": "equations,graphing",
                "missed_topics": "graphing",
            },
        )
        runner.add_result(test_num, "POST /api/schoolwork/grade/record (manual grade)", response.status_code, 200, response.json())
        test_num += 1

        # Test 48: GET /api/schoolwork/grades/{student_id} - Should have grade now
        response = await client.get(
            f"/api/schoolwork/grades/{runner.student_id}",
            params={"instance_id": runner.instance_id}
        )
        grade_data = response.json()
        has_grades = len(grade_data.get("grades", [])) >= 1
        runner.add_result(test_num, f"GET /api/schoolwork/grades/{runner.student_id} (has grades)", response.status_code, 200, grade_data)
        test_num += 1

        # Test 49: DELETE /api/schoolwork/upload/{upload_id} - Delete upload
        if upload_id:
            response = await client.delete(
                f"/api/schoolwork/upload/{upload_id}",
                params={"student_id": runner.student_id, "instance_id": runner.instance_id}
            )
            runner.add_result(test_num, f"DELETE /api/schoolwork/upload/{upload_id}", response.status_code, 200, response.json())
        test_num += 1

        # Test 50: GET /api/schoolwork/uploads/{student_id} - Should be empty after delete
        if upload_id:
            response = await client.get(
                f"/api/schoolwork/uploads/{runner.student_id}",
                params={"instance_id": runner.instance_id}
            ...
            data = response.json()
            is_empty = len(data.get("uploads", [])) == 0
            runner.add_result(test_num, f"GET /api/schoolwork/uploads/{runner.student_id} (empty after delete)", response.status_code, 200, data)
        test_num += 1
